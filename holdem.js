...
if(pix.length<50){pic=board[n].substring(0,1)+".gif";pic_click=SUIT_LINK;}
 write_frame("board"+n,"<table width=100% bgcolor=FFFFFF><tr><td valign=top>"+
  get_card_html(board[n])+"</td></tr></table><a href=\""+pic_click+"\" target=_blank><img src='"+
  pic+"'></a><br><table width=100% height=100% bgcolor=FFFFFF><tr><td></td></tr></table>");
}
function main(){
 ...
 if(players[current_bettor_index].status=="BUST"||players[current_bettor_index].status=="FOLD"){
  ...
  if(current_bettor_index==0){
   ...
   var html="<table width=100%><tr><td colspan=2>"+get_pot_size_html()+
    ...
    "</form></td><td valign=bottom><table"+quick_color+"><tr><td align=center>"+quick_bets+"</td></tr></table></td></tr></table>";
...
if(winners[0])HUMAN_WINS_AGAIN++;
 ...
 var html="<table><tr><td>"+get_pot_size_html()+
  "</td></tr></table><br><b>WINNER! "+my_best_hand_name+". "+winner_text+"</b>"+detail+"<br>"+
  "<form name=f>"+the_buttons+"<input type=button value=Quit onclick='parent.confirm_quit()'></form>";
 write_frame("general",html);
...
function write_player(n,hilite,show_cards,mode){
 ...
 var html="<pre><b><font size=+2>"+button+players[n].name+"</font></b>"
 +" ["+players[n].bankroll+"]"
 +"<div class='cheat'>CHEAT! "+players[n].carda.substring(0,1)+make_readable_rank(players[n].carda.substring(1))+" "+players[n].cardb.substring(0,1)+make_readable_rank(players[n].cardb.substring(1))+"</div>"
...
function get_pot_size_html(){return "<strong>TOTAL POT: "+get_pot_size()+"</strong>";}
...
function write_basic_general(){write_frame("general","<table><tr><td>"+get_pot_size_html()+"</td></tr></table>");}
function write_settings_frame(){
 ...
 write_frame("settings","<pre><center><b><font size=+2>Options</font></b>\n\n<a href='javascript:change_name()'>Your name</a>\n\n<form>Speed <select onchange='parent.set_speed(options[selectedIndex].value,selectedIndex);'>"+speed_options+"</select></form><a target=_blank href=\"help.html\">Help</a></center></pre>");
}
function set_deck(v){
